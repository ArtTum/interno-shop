...
namespace App\Services\ERP\Subscribe;
...
class SubscribeService
{
    ...
    {
        $select = "id, date, full_name, phone, description, doctor, hospital, color, year, month";
        $pagination = prepare_pagination_array($data['page'], $data['per_page']);
        $ordering = [
            'field' => $data['ordering_field'],
            'direction' => $data['ordering_direction']
        ];
        $searchFields = ['full_name', 'phone', 'doctor', 'hospital'];
    ...
    public function fetchCount(): JsonResponse
    {
        return response()->json([
            'success' => true,
            'count' => $this->repository->fetchCountByStatus(0),
        ]);
    }
}