...
<template>
    <div class="mt-4" v-if="params.product_id > 0 && pageData.data?.chartDataLabels">
        <ChartTabsProduct
        ...
        <div class="mt-3" v-if="pageData.data?.dataTable">
            ...
            >
                <template #header>
                    <div>
                        <div class="relative">
                            ...
                            >
                                <button
                                    @click="emits('export-datatable', `${pageData.data?.selectedProductSlug}`), dataTableDropDown = false"
                                    type="button"
                ...
                <template #content>
                    <div
                        class="grid border-t border-stroke py-4.5 px-7 text-black font-bold"
                        :class="pageData.data?.dataTable.header[4] ? 'grid-cols-5' : 'grid-cols-4'"
                    >
                        ...
                        <div class="col-span-1 flex items-center" v-if="pageData.data?.dataTable.header[4]">
    ...
    <div v-else-if="!params.product_id || params.product_id <= 0">
        <div class="mt-3" v-if="pageData.data?.dataTable">
            <CustomTableSecond
                title=""
                class="text-sm"
            >
                <template #header>
                    <div>
                        <div class="relative">
                            ...
                            >
                                <button
                                    @click="emits('export-datatable', 'products'), dataTableDropDown = false"
                                    type="button"
                                    class="flex w-full items-center gap-2 rounded-sm py-1.5 px-4 text-left text-sm hover:bg-gray dark:hover:bg-meta-4"
                                >
                                    <font-awesome-icon :icon="['far', 'file-export']"/>
                ...
                <template #content>
                    <div
                        class="grid grid-cols-6 border-t border-stroke py-4.5 px-7 text-black font-bold"
                        :class="{'!grid-cols-10': params.compared_order_date_from}"
                    >
                        <div class="col-span-1 flex items-center">
                            <p>{{ pageData.data?.dataTable.header[0] }}</p>
                        </div>
                        ...
                        <template v-if="params.compared_order_date_from">
                            ...
                            <div class="col-span-1 flex items-center">
                                <p>{{ pageData.data?.dataTable.header[9] }}</p>
                            </div>
                        </template>
                    </div>
                    ...
                    >
                        <div
                            class="grid grid-cols-6 border-t border-stroke py-4.5 px-7 text-black font-medium"
                            :class="{'!grid-cols-10': params.compared_order_date_from, '!font-bold text-title-md': !params.compared_order_date_from && index === pageData.data?.dataTable.data.length - 1}"
                        >
                            <div class="col-span-1 flex items-center">
                                {{ row[0] }}
                            </div>
                            ...
                            <div class="col-span-1 flex items-center">
                                ...
                                <template v-if="params.compared_order_date_from">
                                     <span
                                         ...
                                         :class="{ 'text-meta-3': usePercentageChange(row[2], row[3]) > 0, 'text-danger': usePercentageChange(row[2], row[3]) < 0 , 'text-warning': usePercentageChange(row[2], row[3]) === 0 }"
                                     >
                                        ({{ usePercentageChange(row[2], row[3]) }}%)

                                        <svg
                                            v-if="usePercentageChange(row[2], row[3]) !== 0"
                                            width="15"
                                            height="15"
                                            viewBox="0 0 15 15"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            :class="{'deg-180': usePercentageChange(row[2], row[3]) < 0}"
                                        >
                                          <g clip-path="url(#clip0_554_43030)">
                                            <path
                                                ...
                                                :fill="usePercentageChange(row[2], row[3]) > 0 ? '#10B981' : '#dc3545'"
                                            />
                                          </g>
                                          <defs>
                            ...
                            <div class="col-span-1 flex items-center">
                                ...
                                <template v-if="params.compared_order_date_from">
                                     <span
                                         class="flex items-center gap-1 text-sm font-medium ml-2"
                                         :class="{ 'text-meta-3': usePercentageChange(row[4], row[5]) > 0, 'text-danger': usePercentageChange(row[4], row[5]) < 0 , 'text-warning': usePercentageChange(row[4], row[5]) === 0 }"
                                     >
                                        ({{ usePercentageChange(row[4], row[5]) }}%)

                                        <svg
                                            v-if="usePercentageChange(row[4], row[5]) !== 0"
                                            width="15"
                                            height="15"
                                            viewBox="0 0 15 15"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            :class="{'deg-180': usePercentageChange(row[4], row[5]) < 0}"
                                        >
                                          <g clip-path="url(#clip0_554_43030)">
                                            <path
                                                d="M13.4731 5.62415H9.95118C9.68868 5.62415 9.46993 5.8429 9.46993 6.1054C9.46993 6.3679 9.68868 6.58665 9.95118 6.58665H12.1168L9.3168 8.4679C9.20743 8.5554 9.0543 8.5554 8.92305 8.4679L6.03555 6.56477C5.57618 6.25852 5.00743 6.25852 4.54805 6.56477L1.1793 8.8179C0.960552 8.97102 0.894927 9.27727 1.04805 9.49602C1.13555 9.62727 1.28868 9.71477 1.46368 9.71477C1.55118 9.71477 1.66055 9.6929 1.72618 9.62727L5.1168 7.37415C5.22618 7.28665 5.3793 7.28665 5.51055 7.37415L8.39805 9.29915C8.85743 9.6054 9.42618 9.6054 9.88555 9.29915L12.9699 7.22102V9.64915C12.9699 9.91165 13.1887 10.1304 13.4512 10.1304C13.7137 10.1304 13.9324 9.91165 13.9324 9.64915V6.1054C13.9762 5.8429 13.7356 5.62415 13.4731 5.62415Z"
                                                :fill="usePercentageChange(row[4], row[5]) > 0 ? '#10B981' : '#dc3545'"
                                            />
                                          </g>
                                          <defs>
                                            <clipPath id="clip0_554_43030">
                            ...
                            <div class="col-span-1 flex items-center">
                                ...
                                <template v-if="params.compared_order_date_from">
                                     <span
                                         class="flex items-center gap-1 text-sm font-medium ml-2"
                                         :class="{ 'text-meta-3': usePercentageChange(row[6], row[7]) > 0, 'text-danger': usePercentageChange(row[6], row[7]) < 0 , 'text-warning': usePercentageChange(row[6], row[7]) === 0 }"
                                     >
                                        ({{ usePercentageChange(row[6], row[7]) }}%)

                                        <svg
                                            v-if="usePercentageChange(row[6], row[7]) !== 0"
                                            width="15"
                                            height="15"
                                            viewBox="0 0 15 15"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            :class="{'deg-180': usePercentageChange(row[6], row[7]) < 0}"
                                        >
                                          <g clip-path="url(#clip0_554_43030)">
                                            <path
                                                d="M13.4731 5.62415H9.95118C9.68868 5.62415 9.46993 5.8429 9.46993 6.1054C9.46993 6.3679 9.68868 6.58665 9.95118 6.58665H12.1168L9.3168 8.4679C9.20743 8.5554 9.0543 8.5554 8.92305 8.4679L6.03555 6.56477C5.57618 6.25852 5.00743 6.25852 4.54805 6.56477L1.1793 8.8179C0.960552 8.97102 0.894927 9.27727 1.04805 9.49602C1.13555 9.62727 1.28868 9.71477 1.46368 9.71477C1.55118 9.71477 1.66055 9.6929 1.72618 9.62727L5.1168 7.37415C5.22618 7.28665 5.3793 7.28665 5.51055 7.37415L8.39805 9.29915C8.85743 9.6054 9.42618 9.6054 9.88555 9.29915L12.9699 7.22102V9.64915C12.9699 9.91165 13.1887 10.1304 13.4512 10.1304C13.7137 10.1304 13.9324 9.91165 13.9324 9.64915V6.1054C13.9762 5.8429 13.7356 5.62415 13.4731 5.62415Z"
                                                :fill="usePercentageChange(row[6], row[7]) > 0 ? '#10B981' : '#dc3545'"
                                            />
                                          </g>
                                          <defs>
                                            <clipPath id="clip0_554_43030">
                            ...
                            <template v-if="params.compared_order_date_from">
                                ...
                                <div class="col-span-1 flex items-center">
                                    ...
                                    <template v-if="params.compared_order_date_from">
                                     <span
                                         class="flex items-center gap-1 text-sm font-medium ml-2"
                                         :class="{ 'text-meta-3': usePercentageChange(row[8], row[9]) > 0, 'text-danger': usePercentageChange(row[8], row[9]) < 0 , 'text-warning': usePercentageChange(row[8], row[9]) === 0 }"
                                     >
                                        ({{ usePercentageChange(row[8], row[9]) }}%)

                                        <svg
                                            v-if="usePercentageChange(row[8], row[9]) !== 0"
                                            width="15"
                                            height="15"
                                            viewBox="0 0 15 15"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            :class="{'deg-180': usePercentageChange(row[8], row[9]) < 0}"
                                        >
                                          <g clip-path="url(#clip0_554_43030)">
                                            <path
                                                d="M13.4731 5.62415H9.95118C9.68868 5.62415 9.46993 5.8429 9.46993 6.1054C9.46993 6.3679 9.68868 6.58665 9.95118 6.58665H12.1168L9.3168 8.4679C9.20743 8.5554 9.0543 8.5554 8.92305 8.4679L6.03555 6.56477C5.57618 6.25852 5.00743 6.25852 4.54805 6.56477L1.1793 8.8179C0.960552 8.97102 0.894927 9.27727 1.04805 9.49602C1.13555 9.62727 1.28868 9.71477 1.46368 9.71477C1.55118 9.71477 1.66055 9.6929 1.72618 9.62727L5.1168 7.37415C5.22618 7.28665 5.3793 7.28665 5.51055 7.37415L8.39805 9.29915C8.85743 9.6054 9.42618 9.6054 9.88555 9.29915L12.9699 7.22102V9.64915C12.9699 9.91165 13.1887 10.1304 13.4512 10.1304C13.7137 10.1304 13.9324 9.91165 13.9324 9.64915V6.1054C13.9762 5.8429 13.7356 5.62415 13.4731 5.62415Z"
                                                :fill="usePercentageChange(row[8], row[9]) > 0 ? '#10B981' : '#dc3545'"
                                            />
                                          </g>
                                          <defs>
                                            <clipPath id="clip0_554_43030">
