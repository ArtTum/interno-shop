    ...
    <form @submit.prevent="emits('submit')">
        ...
        <div class="grid grid-cols-2 gap-9">
            <div class="flex flex-col p-6.5">
                <CustomInput
                    :disabled="emitAction === 'update' && !auth.user_group.permissions_by_name.customers_groups[0].can_edit"
                    v-model="form.name"
                    name="name"
                    label="Name *"
                    type="text"
        ...
        <div class="grid grid-cols-1 gap-9">
            <div class="flex flex-col p-6.5 save-button-fixed">
                <div class="flex ml-auto gap-5">
                    <template v-if="auth.user_group.permissions_by_name.customers_groups[0].can_delete">
                        <CustomButton
                            @click="store.commit('customerGroup/SET_DELETE_MODAL_VALUE', {
                                    value: true,
                                    id: form.id
                                });"
                    ...
                    <template
                        v-if="emitAction !== 'update' || auth.user_group.permissions_by_name.customers_groups[0].can_edit">
                        <CustomButton
