...
const fetchPageData = async () => {
    await store.dispatch('disease/fetchPageData', params.value);
};
...
const pageData = computed(() => store.getters['disease/getPageData']);
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="Հիվանդություններ" :breadcrumb="[
            {path: '/dashboard', title: 'Վահանակ'},
        ]"/>
        <TableActions
            :createRoute="'/diseases/create'"
            :showFilter="false"
                ...
                <tr>
                    ...
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="'diseases/update/' + item.id">
                                <button
                                ...
                                <button
                                    @click="store.commit('disease/SET_DELETE_MODAL_VALUE', {
                                    value: true,
                                    id: item.id
                                });"
                                    class="hover:text-primary"
