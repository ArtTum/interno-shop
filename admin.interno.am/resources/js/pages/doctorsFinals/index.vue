...
const fetchPageData = async () => {
    await store.dispatch('doctorsFinal/fetchPageData', params.value);
};
...
const pageData = computed(() => store.getters['doctorsFinal/getPageData']);
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="ԲԺԻՇԿՆԵՐ ՎԵՐՋՆԱԿԱՆ" :breadcrumb="[
            {path: '/dashboard', title: 'Վահանակ'},
        ]"/>
        <TableActions
            :createRoute="(auth?.superadmin || auth?.user_group?.permissions_by_name.doctors_finals[0].can_add) ? '/doctorsFinal/create' : null"
            :showFilter="false"
        ...
        <CustomTable
            ...
            :columns="[
              { title: '#' },
              { title: 'Անուն ազգանուն հայրանուն' },
              { title: 'Մասնագիտություն' },
              { title: 'Կոչում' },
              { title: 'Աշխատանքի վայր' },
        ...
        >
            <template v-for="(item, index) in pageData.data" :key="index">
                <tr>
                    ...
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[200px] break-words whitespace-normal font-medium text-black">{{ item.full_name }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[200px] break-words whitespace-normal font-medium text-black">{{ item.profession }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[200px] break-words whitespace-normal font-medium text-black">{{ item.degree }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[200px] break-words whitespace-normal font-medium text-black">{{ item.workplace }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[200px] break-words whitespace-normal font-medium text-black">{{ item.other_info }}</span>
                    </td>
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="'doctors-finals/update/' + item.id">
                                <button
                                ...
                                <button
                                    v-if="auth.user_group.permissions_by_name.doctors_finals[0].can_delete"
                                    @click="store.commit('doctorsFinal/SET_DELETE_MODAL_VALUE', {
                                    value: true,
                                    id: item.id
                                });"
                                    class="hover:text-primary"
