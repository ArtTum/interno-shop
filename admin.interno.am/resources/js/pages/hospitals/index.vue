...
const fetchPageData = async () => {
    await store.dispatch('hospital/fetchPageData', params.value);
};
...
const pageData = computed(() => store.getters['hospital/getPageData']);
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="Հիվանդանոցներ" :breadcrumb="[
            {path: '/dashboard', title: 'Վահանակ'},
        ]"/>
        <TableActions
            :createRoute="'/hospitals/create'"
            :showFilter="false"
                ...
                <tr>
                    ...
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="'hospitals/update/' + item.id">
                                <button class="hover:text-primary" title="Edit">
                                    <font-awesome-icon :icon="['far', 'pen-to-square']"/>
                                </button>
                            </RouterLink>
                            <button
                                @click="store.commit('hospital/SET_DELETE_MODAL_VALUE', {value: true, id: item.id});"
