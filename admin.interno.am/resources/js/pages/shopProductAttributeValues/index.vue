...
const pageData = computed(() => store.getters['shopProductAttributeValue/getPageData']);
...
const updateQueryParams = async () => {
    await router.push({query: {...route.query, ...params.value, type: undefined}});
};

const fetchPageData = async () => {
    params.value.type = type.value;
    await store.dispatch('shopProductAttributeValue/fetchPageData', params.value);
};
...
watch(type, async () => {
    params.value = {...params.value, type: type.value, page: 1, search: '', status: -1};
    await fetchPageData();
});
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault :pageTitle="config.title" :breadcrumb="[{path: '/', title: 'Dashboard'}]"/>
        <TableActions
            :createRoute="canAdd ? `${config.route}/create` : ''"
            :showFilter="true"
                ...
                <tr>
                    ...
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="`${config.route}/update/${item.id}`">
                            ...
                            <button
                                v-if="canDelete"
                                @click="store.commit('shopProductAttributeValue/SET_DELETE_MODAL_VALUE', {value: true, id: item.id})"
        ...
        <DeleteModal
            @fetch="fetchPageData()"
            action-variable="shopProductAttributeValue/delete"
            getter-variable="shopProductAttributeValue/getDeleteModelValue"
            mutation-variable="shopProductAttributeValue/SET_DELETE_MODAL_VALUE"
        />
    </DefaultLayoutComponent>
</template>
