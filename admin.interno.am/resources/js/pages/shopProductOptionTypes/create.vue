...
const form = reactive({name: '', status: true, sort_order: 0, errors: {}});

const submit = async () => {
    try {
        form.errors = {};
        const response = await store.dispatch('shopProductOptionType/create', form);
        await router.push(`/shop-product-option-types/update/${response.data.data.id}`);
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="Create product type" :breadcrumb="[{path: '/', title: 'Dashboard'}, {path: '/shop-product-option-types', title: 'Product Types'}]"/>
        <div class="rounded-sm border border-stroke bg-white shadow-default">
            ...
            <form @submit.prevent="submit">
                ...
                <div class="grid grid-cols-3 gap-6 p-6 max-md:grid-cols-1">
                    ...
                    <div class="flex items-end pb-2"><Switch @change="(value) => form.status = value" :value="form.status" id="type_status" label="Active"/></div>
                </div>
