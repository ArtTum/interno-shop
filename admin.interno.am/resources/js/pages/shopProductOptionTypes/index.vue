...
const pageData = computed(() => store.getters['shopProductOptionType/getPageData']);
...
const fetchPageData = async () => {
    await store.dispatch('shopProductOptionType/fetchPageData', params.value);
};
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="Product Types" :breadcrumb="[{path: '/', title: 'Dashboard'}]"/>
        <TableActions
            :createRoute="canAdd ? '/shop-product-option-types/create' : ''"
            :showFilter="true"
        ...
        <CustomTable
            :main-search="{visibility: true, placeholder: 'Search ...', tooltip: {button: {showingType: 'info'}, text: 'Name'}}"
                ...
                <tr>
                    ...
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="`/shop-product-option-types/update/${item.id}`">
                            ...
                            <button
                                v-if="canDelete"
                                @click="store.commit('shopProductOptionType/SET_DELETE_MODAL_VALUE', {value: true, id: item.id})"
        ...
        <DeleteModal
            @fetch="fetchPageData()"
            action-variable="shopProductOptionType/delete"
            getter-variable="shopProductOptionType/getDeleteModelValue"
            mutation-variable="shopProductOptionType/SET_DELETE_MODAL_VALUE"
        />
    </DefaultLayoutComponent>
</template>
