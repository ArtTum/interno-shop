...
const fetchPageData = async () => {
    await store.dispatch('smsBaza/fetchPageData', params.value);
};
...
const fetchIndexParams = async () => {
    indexParams.value = await store.dispatch('smsBaza/fetchIndexParams', {
        dontNeedLoading: true
    });
};

const pageData = computed(() => store.getters['smsBaza/getPageData']);
...
<template>
    <DefaultLayoutComponent>
        <BreadcrumbDefault pageTitle="SMS ԲԱԶԱ" :breadcrumb="[
            {path: '/dashboard', title: 'Վահանակ'},
        ]"/>
        <TableActions
            :createRoute="(auth?.superadmin || auth?.user_group?.permissions_by_name.sms_bazas[0].can_add) ? '/sms-bazas/create' : null"
            :showFilter="true"
                ...
                <tr>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="font-medium text-black">
                            {{ item.call_date ? item.call_date.slice(0, 10).split('-').reverse().join('.') : '' }}
                        </span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="font-medium text-black">{{ item.patient_full_name }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="font-medium text-black">{{ item.phone }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="font-medium text-black">{{ item.other_phone }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[140px] break-words whitespace-normal font-medium text-black">{{ item.disease }}</span>
                    </td>
                    <td class="py-5 px-4 pl-9 xl:pl-11">
                        <span class="block w-[160px] break-words whitespace-normal font-medium text-black">{{ item.medical_and_doctor }}</span>
                    </td>
                    <td class="py-5 px-4">
                        <div class="flex items-center space-x-3.5">
                            <RouterLink :to="'sms-bazas/update/' + item.id">
                                <button class="hover:text-primary" title="Edit">
                                    <font-awesome-icon :icon="['far', 'pen-to-square']"/>
                                </button>
                            </RouterLink>
                            <button
                                v-if="auth.user_group.permissions_by_name.sms_bazas[0].can_delete"
                                @click="store.commit('smsBaza/SET_DELETE_MODAL_VALUE', {value: true, id: item.id});"
