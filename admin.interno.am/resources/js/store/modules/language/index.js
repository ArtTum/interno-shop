...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('languages/fetch', {
            params
        });
    ...
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('languages/fetch-by-field', {
            params
        });
    ...
    async delete({state}) {
        return await baseHttp.delete(`languages/delete/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`languages/insert`, params);
    },
