...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('lead-projects/fetch', {
            params
        });
    ...
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('lead-projects/fetch-by-field', {
            params
        });
    ...
    async fetchParams({commit}, params) {
        const res = await baseHttp.get('lead-projects/fetch-params', {params});
        commit('SET_PARAMS', res.data);
    },

    async delete({state}) {
        return await baseHttp.delete(`lead-projects/${state.deletingActionApi}/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`lead-projects/insert`, params);
    },

    async update({}, params) {
        return await baseHttp.put(`lead-projects/update`, params);
    },
};
