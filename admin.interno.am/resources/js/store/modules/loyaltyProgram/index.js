...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('loyalty-programs/fetch', {
            params
        });
    ...
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('loyalty-programs/fetch-by-field', {
            params
        });
    ...
    async fetchParams({commit}, params) {
        const res = await baseHttp.get('loyalty-programs/fetch-params', {params});
        commit('SET_PARAMS', res.data);
    },

    async delete({state}) {
        return await baseHttp.delete(`loyalty-programs/${state.deletingActionApi}/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`loyalty-programs/insert`, params);
    },

    async update({}, params) {
        return await baseHttp.put(`loyalty-programs/update`, params);
    },
};
