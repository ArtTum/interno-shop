import baseHttp from "@store/api.js";

const state = () => ({
    deleteModalValue: null,
    deletingItemId: null,
    deletingActionApi: 'delete',
    deletingText: null,
    editData: [],
    params: [],
});

const getters = {
    getDeleteModelValue: (state) => state.deleteModalValue,
    getDeletingText: (state) => state.deletingText,
    getEditData: (state) => state.editData,
    getParams: (state) => state.params,
};

const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('menus/fetch', {
            params
        });

        return res.data;
    },

    async fetchByField({commit}, params) {
        const res = await baseHttp.get('menus/fetch-by-field', {
            params
        });
        commit('SET_EDIT_DATA', res.data.data);
        return res.data.data;
    },

    async fetchParams({commit}, params) {
        const res = await baseHttp.get('menus/fetch-params', {params});
        commit('SET_PARAMS', res.data);
    },

    async delete({state}) {
        return await baseHttp.delete(`menus/${state.deletingActionApi}/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`menus/insert`, params);
    },

    async update({}, params) {
        return await baseHttp.put(`menus/update`, params);
    },

    async updatePriority({}, params) {
        return await baseHttp.patch(`menus/update-priority`, params);
    },

    async saveMenuStyles({}, params) {
        return await baseHttp.patch(`menus/save-menu-styles`, params);
    },

    async upload({}, params) {
        return await baseHttp.post(`menus/upload`, params);
    },

    async exportFile({}, params) {
        return await baseHttp.get(`menus/export`, {
            params,
            responseType: "blob",
        });
    },
};

const mutations = {
    SET_DELETE_MODAL_VALUE(state, data) {
        if (data.value !== undefined) {
            state.deleteModalValue = data.value;
        }
        if (data.id !== undefined) {
            state.deletingItemId = data.id;
        }
        if (data.deletingActionApi !== undefined) {
            state.deletingActionApi = data.deletingActionApi;
            state.deletingText = data.deletingText;
        }
    },
    SET_EDIT_DATA(state, data) {
        state.editData = data;
    },
    SET_PARAMS(state, params) {
        state.params = params;
    },
};

const menu = {
    namespaced: true,
    state,
    getters,
    actions,
    mutations,
};

export default menu;

