...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('offers/fetch', {
            params
        });
    ...
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('offers/fetch-by-field', {
            params
        });
    ...
    async fetchParams({commit}) {
        const res = await baseHttp.get('offers/fetch-params');
        commit('SET_PARAMS', res.data);
    },

    async delete({state}) {
        return await baseHttp.delete(`offers/delete/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`offers/insert`, params);
    },

    async update({}, params) {
        return await baseHttp.put(`offers/update`, params);
    },
    async autocomplete({commit}, params) {
        const res = await baseHttp.get('offers/autocomplete', {params});
