...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('shop-product-attribute-values/fetch', {params});
        commit('SET_PAGE_DATA', res.data);
        return res.data;
    },
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('shop-product-attribute-values/fetch-by-field', {params});
        commit('SET_EDIT_DATA', res.data.data);
        return res.data;
    },
    async create({}, params) {
        return await baseHttp.post('shop-product-attribute-values/insert', params);
    },
    async update({}, params) {
        return await baseHttp.put('shop-product-attribute-values/update', params);
    },
    async delete({state}) {
        return await baseHttp.delete(`shop-product-attribute-values/delete/${state.deletingItemId}`);
    },
};
