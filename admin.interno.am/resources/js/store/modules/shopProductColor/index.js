...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('shop-product-colors/fetch', {params});
        commit('SET_PAGE_DATA', res.data);
        return res.data;
    },
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('shop-product-colors/fetch-by-field', {params});
        commit('SET_EDIT_DATA', res.data.data);
        return res.data;
    },
    async create({}, params) {
        return await baseHttp.post('shop-product-colors/insert', params);
    },
    async update({}, params) {
        return await baseHttp.put('shop-product-colors/update', params);
    },
    async delete({state}) {
        return await baseHttp.delete(`shop-product-colors/delete/${state.deletingItemId}`);
    },
};
