...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('shop-product-option-types/fetch', {params});
        commit('SET_PAGE_DATA', res.data);
        return res.data;
    },
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('shop-product-option-types/fetch-by-field', {params});
        commit('SET_EDIT_DATA', res.data.data);
        return res.data;
    },
    async create({}, params) {
        return await baseHttp.post('shop-product-option-types/insert', params);
    },
    async update({}, params) {
        return await baseHttp.put('shop-product-option-types/update', params);
    },
    async delete({state}) {
        return await baseHttp.delete(`shop-product-option-types/delete/${state.deletingItemId}`);
    },
};
