...
const actions = {
    async fetchPageData({commit}, params) {
        const res = await baseHttp.get('users/fetch', {
            params
        });
    ...
    async fetchByField({commit}, params) {
        const res = await baseHttp.get('users/fetch-by-field', {
            params
        });
    ...
    async fetchParams({commit}) {
        const res = await baseHttp.get('users/fetch-params', {params: {dontNeedLoading: true}});
        commit('SET_PARAMS', res.data);
    },

    async delete({state}) {
        return await baseHttp.delete(`users/delete/${state.deletingItemId}`);
    },

    async create({}, params) {
        return await baseHttp.post(`users/insert`, params);
    },

    async update({}, params) {
        return await baseHttp.put(`users/update`, params);
    },

    async upload({}, params) {
        return await baseHttp.post(`users/upload`, params);
    },

    async exportFile({}, params) {
        return await baseHttp.get(`users/export`, {
            params,
            responseType: "blob",
        });
    },
    async autocomplete({commit}, params) {
        const res = await baseHttp.get('users/autocomplete', {params});
